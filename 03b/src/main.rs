...
fn count_trees(map: &HashMap<Position, Tile>, width: i32, slope: Position) -> usize {
    let mut position: Position = (0, 0).into();
    ...
    while map.contains_key(&position) {
        ...
        position = ((position.x + slope.x) % width, position.y + slope.y).into();
    }
...
fn main() {
    ...
    let trees = count_trees(&map, width, (1, 1).into()) *
        count_trees(&map, width, (3, 1).into()) *
        count_trees(&map, width, (5, 1).into()) *
        count_trees(&map, width, (7, 1).into()) *
        count_trees(&map, width, (1, 2).into());
